...
#include "SkeletalMeshMergerLibrary.h"
...
static void ToMergeParams(const TArray<FSkeletalMeshMergeSectionMapping>& InSectionMappings,
...
static void ToMergeParams(const TArray<FSkeletalMeshMergeUVTransformMapping>& InUVTransformsPerMesh,
	TArray<FSkelMeshMergeUVTransforms>& OutUVTransformsPerMesh)
{
	if (InUVTransformsPerMesh.Num() > 0)
	{
		...
		for (int32 i = 0; i < InUVTransformsPerMesh.Num(); ++i)
		{
			...
			const TArray<FSkeletalMeshMergeUVTransform>& InUVTransforms = InUVTransformsPerMesh[i].UVTransformsPerMesh;
...
USkeletalMesh* USkeletalMeshMergerLibrary::MergeMeshes(const FSkeletalMeshMergeParams& Params)
{
	TArray<USkeletalMesh*> MeshesToMergeCopy = Params.MeshesToMerge;
    MeshesToMergeCopy.RemoveAll(
